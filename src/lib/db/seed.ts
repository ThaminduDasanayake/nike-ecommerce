...
async function seed() {
  try {
    ...
    const runningCat = (
      await db.select().from(categories).where(eq(categories.slug, 'running-shoes'))
    )[0];
    const lifestyleCat = (
      await db.select().from(categories).where(eq(categories.slug, 'lifestyle'))
    )[0];
    const summer = (
      await db.select().from(collections).where(eq(collections.slug, 'summer-25'))
    )[0];
    const newArrivals = (
      await db.select().from(collections).where(eq(collections.slug, 'new-arrivals'))
    )[0];
    ...
    for (let i = 0; i < productNames.length; i++) {
      ...
      for (const color of colorChoices) {
        for (const size of sizeChoices) {
          ...
          const discountedNum =
            Math.random() < 0.3 ? Number((priceNum - randInt(5, 25)).toFixed(2)) : null;
