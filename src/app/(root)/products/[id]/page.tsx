...
import { Card, CollapsibleSection, ColorSwatches, ProductGallery, SizePicker } from '@/components';
...
function NotFoundBlock() {
  return (
    <section className="border-light-300 bg-light-100 mx-auto max-w-3xl rounded-xl border p-8 text-center">
      ...
      <div className="mt-6">
        <Link
          href="/products"
          className="bg-dark-900 text-body-medium text-light-100 inline-block rounded-full px-6 py-3 transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]"
        >
          Browse Products
        </Link>
      </div>
...
async function ReviewsSection({ productId }: { productId: string }) {
  ...
  return (
    <CollapsibleSection
      title={`Reviews (${count})`}
      rightMeta={
        <span className="text-dark-900 flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((i) => (
            <Star
              key={i}
    ...
    >
      {reviews.length === 0 ? (
        ...
        <ul className="space-y-4">
          {reviews.slice(0, 10).map(({ id, author, rating, title, content, createdAt }) => (
            <li key={id} className="border-light-300 rounded-lg border p-4">
              <div className="mb-1 flex items-center justify-between">
                <p className="text-body-medium text-dark-900">{author}</p>
                <span className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${i <= rating ? 'fill-[--color-dark-900]' : ''}`}
                    />
                  ))}
                </span>
              </div>
              {title && <p className="text-body-medium text-dark-900">{title}</p>}
              {content && <p className="text-body text-dark-700 mt-1 line-clamp-[8]">{content}</p>}
              <p className="text-caption text-dark-700 mt-2">
  ...
  if (!data) {
    return (
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <nav className="text-caption text-dark-700 py-4">
      ...
      <section className="grid grid-cols-1 gap-10 lg:grid-cols-[1fr_480px]">
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="flex items-center gap-3">
            ...
            {compareAt && (
              <>
                ...
                {discount !== null && (
                  <span className="border-light-300 text-caption rounded-full border px-2 py-1 text-[--color-green]">
                    {discount}% off
                  </span>
                )}
              </>
          ...
          <div className="flex flex-col gap-3">
            <button className="bg-dark-900 text-body-medium text-light-100 flex items-center justify-center gap-2 rounded-full px-6 py-4 transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]">
              <ShoppingBag className="h-5 w-5" />
              Add to Bag
            </button>
            <button className="border-light-300 text-body-medium text-dark-900 hover:border-dark-500 flex items-center justify-center gap-2 rounded-full border px-6 py-4 transition focus:outline-none focus-visible:ring-2 focus-visible:ring-[--color-dark-500]">
          ...
          <section className="mt-16">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {Array.from({ length: 3 }).map((_, i) => (
                <div key={i} className="bg-light-200 h-64 animate-pulse rounded-xl" />
              ))}
            </div>
          </section>
        }
